...
// const MODEL_PATH: &str = "./fm-classifier.onnx";
...
fn predict(model: &TractModel, img: SmallVec<[TValue; 4]>) {
    let _res = model.run(img);
}
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    //     .into();
    let img: Tensor = tract_ndarray::Array4::from_shape_fn(IMG_DIMS, |(_, c, y, x)| {
        img[(x as _, y as _)][c] as f32
    })
    .into();

    // ****
    let img = tvec!(img.into());
    // let mut t = vec![img.clone(); 10000];
    ...
    group.bench_function("model predict", |b| {
        b.iter(|| predict(black_box(&model), black_box(img.clone())))
    });
    group.finish();
}
